...
class RandomProperties: XCTestCase {
    func testAll() {
        property("randoms can be hex serialized and deserialized") <- forAll { (rnd: Random) in
            return Random(hexEncoded: rnd.hexEncodedString()) == rnd
        }

        property("hex encoding is 32 characters") <- forAll { (rnd: Random) in
            return rnd.hexEncodedString().count == 32
        }
    }
}
